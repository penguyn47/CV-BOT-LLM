...
export async function POST(request: NextRequest) {
	try {
		...
		// Tạo prompt cho AI với thông tin CV và focus vào học vấn
...
- Thời gian: ${edu.startDate} - ${edu.endDate || 'Hiện tại'}
		...
		const prompt = `Bạn là một chuyên gia tư vấn về CV và học vấn. Dưới đây là thông tin CV hiện tại của ứng viên:
...
${resumeData.workExperiences?.map((exp: any) => `
- Công ty: ${exp.company}
- Vị trí: ${exp.position}
...
Hãy trả lời câu hỏi của người dùng về học vấn một cách chuyên nghiệp và hữu ích. Tập trung vào việc tư vấn về cách cải thiện phần học vấn trong CV. Trả lời bằng tiếng Việt và sử dụng xuống dòng để dễ đọc.
...
Câu hỏi của người dùng: ${message}`
		...
		const response = await fetch('https://api.openai.com/v1/chat/completions', {
			...
			body: JSON.stringify({
				...
				messages: [
					{
						role: 'system',
						content: 'Bạn là một chuyên gia tư vấn CV chuyên về phần học vấn. Hãy trả lời bằng tiếng Việt một cách chuyên nghiệp và hữu ích. Sử dụng xuống dòng để tách các ý chính và làm cho câu trả lời dễ đọc hơn.',
					},
					{
						role: 'user',
						content: prompt,
		...
		console.error('Error in education chat API:', error)
		return NextResponse.json(
