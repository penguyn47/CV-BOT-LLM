...
export async function POST(request: NextRequest) {
	try {
		...
		// Tạo prompt cho AI với thông tin CV và focus vào kỹ năng
		const skillsInfo = resumeData.skills?.join(', ') || 'Chưa có kỹ năng'

		const prompt = `Bạn là một chuyên gia tư vấn về CV và kỹ năng. Dưới đây là thông tin CV hiện tại của ứng viên:
...
Hãy trả lời câu hỏi của người dùng về kỹ năng một cách chuyên nghiệp và hữu ích. Tập trung vào việc tư vấn về cách cải thiện phần kỹ năng trong CV. Trả lời bằng tiếng Việt và sử dụng xuống dòng để dễ đọc.
...
Câu hỏi của người dùng: ${message}`
		...
		const response = await fetch('https://api.openai.com/v1/chat/completions', {
			...
			body: JSON.stringify({
				...
				messages: [
					{
						role: 'system',
						content: 'Bạn là một chuyên gia tư vấn CV chuyên về phần kỹ năng. Hãy trả lời bằng tiếng Việt một cách chuyên nghiệp và hữu ích. Sử dụng xuống dòng để tách các ý chính và làm cho câu trả lời dễ đọc hơn.',
					},
					{
						role: 'user',
						content: prompt,
		...
		console.error('Error in skills chat API:', error)
		return NextResponse.json(
