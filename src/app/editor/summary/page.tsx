	...
	const handleImproveHint = async (hint: Hint) => {
		...
		try {
			...
			if (response.ok) {
				...
				console.error('Không thể cải thiện hồ sơ:', await response.json())
				alert('Đã xảy ra lỗi khi cải thiện tổng quan.')
			}
		} catch (error) {
			...
			<div className="col-span-1">
				...
				<div className="p-4">
					<div className="mb-4 flex items-center justify-between">
						<h2 className="text-lg font-semibold text-gray-800">Gợi ý cải thiện tổng quan</h2>
						<button
										...
										<div className="flex items-center space-x-2">
											...
											{hint.type === 'notice' && (
												<button
													...
													className="rounded-md bg-blue-600 px-2 py-1 text-sm font-medium text-white transition-colors duration-150 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600"
													title="Cải thiện tổng quan"
												>
													Cải thiện ngay
												</button>
											)}
				...
				<OneStepNavBar linkPrev={`/editor/skills?resumeId=${resumeId}`} linkNext={`/list/resumes`} />
			</div>
